...
The blacklist contents stay the same as the previous whitelist —
they're the sign-in-adjacent libraries we want lying about VM
presence. The semantics flip from "patch THESE" to "don't patch
THESE":

  - Libs IN the blacklist → NOT patched → cstring stays
    "kern.hv_vmm_present" → kernel returns ENOENT → cache stays 0 →
    lib thinks "not in a VM".  Used for sign-in / device-likeness
    consumers (AAAFoundation, AuthKit, IDSFoundation,
    MobileActivation, …).
  - Libs NOT in the blacklist → ARE patched → cstring becomes
    "kern.Xv_vmm_present" → kernel returns 1 → cache stores 1 →
    lib thinks "in a VM" (same as stock).  This covers
    libMobileGestalt, PhotoFoundation, AirPlaySupport, VisionKitCore,
    CoreVideo (graphics), plus CoreML, Espresso, AppleNeuralEngine,
    CoreRE, RenderBox, WebGPU, caulk, IOSurfaceAccelerator (compute
    / accel fast paths) — everything we used to explicitly NOT
    patch under the previous whitelist design.

Coverage caveat: an undiscovered sign-in caller that ISN'T in the
blacklist gets patched and ends up thinking "in a VM" — wrong for
sign-in. The blacklist therefore must be reasonably complete. The
previous whitelist design had the same coverage requirement; the
kernel-rename's contribution is that it lets us hand out a clean
ENOENT (handled identically by every defensive post-call check) to
the libs we DO want lying, instead of relying on each caller's
specific error-handling idiom.

Why a cstring patch and not a `bl _sysctlbyname` rewrite:
  - One byte per dylib at a deterministic offset; no instruction-shape
    matching, no capstone window.
  - Doesn't modify executable code, only string data — the kernel
    call still happens (with the wrong name), so any sysctl-tracing
    infrastructure still sees activity.
  - Idempotent: re-running on a DSC that already has the mangle
    finds the mangled cstrings and re-attests their pages (since the
    DSC's chunk-level CodeDirectory needs the slot hash updated for
    every page that contains a mangled byte).
"""
