...
Coverage caveat
---------------
This module handles DSC dylibs only. The 6 standalone Mach-O binaries
patched over SSH by `cfw_patch_hv_vmm.py` (`cfw_install_jb.sh`
JB-3.5 step) have their own LC_CODE_SIGNATURE entries and are NOT
re-attested here. The currently-running daemons don't crash because
the in-memory image was already TXM-validated at boot, but a restart
that re-loads from disk could trigger the same `Invalid Page` SIGKILL
seen pre-re-attestation in the DSC case. If those daemons start
crashing post-restart, port this logic to read each Mach-O's
LC_CODE_SIGNATURE (cmd=0x1D), parse the embedded CS_SuperBlob, and
recompute slot hashes the same way.
"""
